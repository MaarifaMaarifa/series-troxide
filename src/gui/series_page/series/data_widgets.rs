...
pub fn series_metadata<'a>(
    ...
    if let Some(image_bytes) = image_bytes {
        let image_handle = iced::widget::image::Handle::from_bytes(image_bytes);
    ...
    let rating_widget = rating_widget(series_information.rating.average);
...
pub fn status_widget<'b>(
    ...
    let mut status_text = text(series_status.to_string());
...
pub fn series_type_widget<'b>(
    ...
    if let Some(kind) = series_info.kind.clone() {
...
pub fn rating_widget(rating: Option<f32>) -> Element<'static, Message> {
    if let Some(average_rating) = rating {
    ...
    if let Some(webchannel) = series_info.web_channel.as_ref() {
        ...
        return data_grid.push(
            GridRow::new()
                ...
                .push(text(webchannel.name.clone())),
        );
    };
    data_grid
}
...
pub fn next_episode_to_air_widget(
        ...
        next_episode_to_air.map(|episode| (episode, episode.release_time().ok()))
    {
        ...
        container(row![clock_icon, text].align_y(Alignment::Center).spacing(5))
